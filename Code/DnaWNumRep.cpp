...
DnaWNumRep::DnaWNumRep(std::string read , std::vector<std::vector<double>> *rep){
    std::vector<std::vector<double>>(2,std::vector<double>(read.size(),0.0)).swap((*rep));
    ...
    for (int i = 0; i < (int)read.size(); i++){
        if(read.substr(i,1) == "A" || read.substr(i,1) == "a"){
            ...
            (*rep)[0][i] = (0.0 + 1.0 + -1.0)/3.0;
            (*rep)[1][i] = (-1.0 + 0.0 + 0.0)/3.0;
        }
        else if(read.substr(i,1) == "H" || read.substr(i,1) == "h"){
            (*rep)[0][i] = (0.0 + 1.0 + 1.0)/3.0;
            (*rep)[1][i] = (-1.0 + 0.0 + 1.0)/3.0;
        }
            ...
            (*rep)[0][i] = (1.0 + -1.0 + 0.0)/3.0;
            (*rep)[1][i] = (0.0 + 0.0 + 1.0)/3.0;
        }
        else {
            (*rep)[0][i] = (0.0 + 1.0 + -1.0 + 0.0)/4.0;
            (*rep)[1][i] = (-1.0 + 0.0 + 0.0 + 1.0)/4.0;
        }
    }
};
